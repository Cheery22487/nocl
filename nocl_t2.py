...
def generate_ho_dvr(parameter):
    ...
    x_diag, x_eigen, x_eigen_tr, x_eigenvalues = diagonalize2(x_mat)    #The resulting diagonal matrix is still the matrix representation of the position operator but in position space
...
##Generation of the equidistant momentum grid
...
def generate_legendre_dvr(parameter):
    ...
    x_diag, x_eigen, x_eigen_tr, x_eigenvalues = diagonalize2(x_mat)
...
##        krylov_basis[0][i] = psi[i]
    ...
    h_lanczos_diag, ev, ev_tr,_ = diagonalize2(h_lanczos)
...
def generate_I(x_p,total_dim):
    ...
    n = 0
#    red_mass_v = (1/14 + 1/16) / 1822.7
#    red_mass_d = (1/30 + 1/35) / 1822.7
    red_mass_v = (1/14 + 1/16) / 1836
    red_mass_d = (1/30 + 1/35) / 1836
...
def generate_v_nocl_s1(x_p,total_dim):
    ...
    for i in range(len(v_temp)):
        ...
        #if round(point[0],2) == 2.13 and round(point[2],2) == -0.60:
        #if round(point[1],2) == 4.31 and round(point[2],2) == -0.60:
        #if round(point[0],2) == 2.13 and round(point[1],2) == 4.31:
        #    point.append(v_temp[i])
        #    v_points[ccc] = point
        #    ccc += 1
...
def main():  
    ...
    total_dim = 1
    dims = []
    my_v = (1/14 + 1/16) / 1836                                 #reduzierte Masse von NO
    my_d = (1/30 + 1/35) / 1836                                 #reduzierte Masse vom Massenschwerpunkt NO und Cl
    ...
    psi = generate_psi(total_dim)   ### Initial guess der Wellenfunktion
    ...
    while tottime < 1984:
        tstep_alt = int(tottime + 1) - tottime
        tstep = min(tstep2,tstep_alt)
        print("--")
...
##
##            np.savetxt("ovl2.txt", overlap00[2:])
##            np.savetxt("enew2.txt", en_ew[1:])
##            np.savetxt("xew2.txt",x_ew[2:])
...
##        overlap = np.vdot(psi_back,psi_curr)
##        overlap = np.absolute(overlap)
##        print(f"Overlap: {overlap}")
##
##        if (1-overlap) < 10**(-8):

##        overlapppp = psi_curr - psi_back
##        overlapppp_norm = np.linalg.norm(overlapppp)
##        print(f"Overlap_norm: {overlapppp_norm}")
##        if overlapppp_norm < 10**(-13):
        ...
        if ovvv < 10**(-24):
            ...
            if round(tstep,3) == round(tstep2,3):
                tstep2 *= 1.1
            tstep2 = min(tstep2,1)
            overlap0 = np.vdot(psi_next,psi0)
...
##            smallerstep = False
    ...
    #print(overlap00[2:])
    #print(en_ew)
    np.savetxt("ovl12.txt", overlap00[2:])
    np.savetxt("ovlabs12.txt", overlap000[1:])
    np.savetxt("wf_final12.txt",psi_next)
    np.savetxt("wf_final_sanity12.txt",psi_curr)
##    np.savetxt("enew6.txt", en_ew[1:])
##    np.savetxt("xew6.txt",x_ew[2:])
    exit()
